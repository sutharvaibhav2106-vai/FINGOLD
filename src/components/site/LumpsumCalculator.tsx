...
export function LumpsumCalculator() {
  ...
  const yearlyData = useMemo(() => {
    const data = [];
    const P = Math.max(100, lumpsumAmount);
    ...
    return data;
  }, [lumpsumAmount, duration, expectedReturn]);
  ...
  return (
    <div className="mx-auto max-w-6xl rounded-3xl border border-border/40 bg-background/50 p-6 md:p-10 shadow-soft backdrop-blur-xl">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    className="w-36 rounded-xl border border-border bg-white py-2 pl-7 pr-3 text-right font-semibold text-foreground shadow-sm focus:outline-none focus:ring-1 focus:ring-ring"
                  />
                </div>
              </div>
              <input
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    max="20"
                    value={duration}
                    onChange={(e) => setDuration(Math.min(20, Math.max(1, parseInt(e.target.value) || 0)))}
            ...
            <div className="flex items-center gap-4 pt-4">
              <button
                onClick={() => {
                  // Interactive calculation flash or animation trigger can go here
                }}
              ...
              <button
                onClick={handleReset}
                className="flex items-center gap-2 rounded-2xl border border-border/80 hover:border-foreground/40 bg-white/60 px-6 py-4 font-semibold text-foreground/75 transition-colors cursor-pointer text-base"
              >
        ...
        <div className="flex flex-col justify-between space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-2xl border border-border/30 bg-background/30 p-4">
              <span className="text-xs font-semibold text-foreground/50 uppercase tracking-wider block">Invested Amount</span>
              <span className="text-xl font-bold text-foreground block mt-1">{formatINR(results.totalInvested)}</span>
            </div>
            <div className="rounded-2xl border border-border/30 bg-background/30 p-4">
              <span className="text-xs font-semibold text-foreground/50 uppercase tracking-wider block">Maturity Value</span>
              <span className="text-xl font-bold text-foreground block mt-1" style={{ color: "var(--bronze)" }}>{formatINR(results.totalValue)}</span>
            </div>
          </div>

          {/* Bar Chart Container */}
          <div className="rounded-3xl border border-[#D4AF37]/25 bg-white/80 p-6 shadow-sm flex flex-col justify-between flex-grow min-h-[350px] overflow-hidden max-w-full">
            ...
            <div className="overflow-x-auto max-w-full pb-3 mt-4 scrollbar-thin">
              <div 
                ...
                style={{ minWidth: `${Math.max(450, duration * 35)}px` }}
              >
                ...
                {yearlyData.map((d) => {
                  ...
                  return (
                    <div key={d.year} className="flex-1 flex flex-col items-center group relative h-full justify-end min-w-[28px]">
                      ...
                      <div
                        className="w-full rounded-t-lg overflow-hidden flex flex-col justify-end shadow-sm hover:brightness-105 transition-all duration-1000 ease-out cursor-pointer"
                        style={{ height: isMounted ? `${barHeightPct}%` : "0%", minHeight: "4px" }}
                      >
                        {/* Estimated Returns (Teal/Green) */}
                        <div
                          className="w-full bg-[#1A535C] transition-all duration-500"
                          style={{ height: `${returnsHeightPct}%` }}
                        />
                        {/* Saved Amount (Yellow/Gold) */}
                        <div
                          className="w-full bg-[#F4D03F] transition-all duration-500"
      ...
      {showAllReturns && (
        <div className="mt-6 overflow-hidden rounded-3xl border border-[#D4AF37]/25 bg-white/90 p-6 shadow-soft animate-reveal">
          <h3 className="text-display text-2xl mb-4 font-semibold text-foreground">Detailed Yearly Growth Breakdown</h3>
